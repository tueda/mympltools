...
function version_bump() {
  ...
  sed -i "s|pip install $urlbase@.*|pip install $urlbase@$1|" README.md
  sed -i "s|poetry add $urlbase@.*|poetry add $urlbase@$1|" README.md
...
# The default next_version is determined as CalVer (YY.MM.MICRO),
# see https://calver.org/.
current_version=$(poetry version -s)
patch_version=0
while :; do
  next_version=$(date '+%-y.%-m').$patch_version
  if [[ -z $(git tag -l "$next_version") ]]; then
    break
  fi
  ((patch_version++)) || :
done
